...
- dring							Toggle the use of Patronus Ring in your engaged melee set.
...
function get_sets()
	...
	sets.idle = set_combine(sets.melee.eva,{feet = "Areion Boots"}) -- Movement
	...
	dring = false														-- true = use dring in engaged set
...
function equip_engaged()
	...
	if dring then equip({right_ring="Patronus Ring"}) end
end	 
...
function self_command(command)
	...
	elseif command == 'dring' then
		if dring == false then
			dring = true
			windower.add_to_chat(122,'Using Patronus Ring in melee set.')
		else
			dring = false
