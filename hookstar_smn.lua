...
- pet_midcast and pet_aftercast don't work on DSP, that was fucking fun to find out.
...
sets.idle={ -- refresh, mp, pdt, mdt all that. Could make more sets and toggle between them if you like (and know how to set that up)
    main		= "Claustrum",
    sub			= "Reign Grip",   --"Magic Strap",--ZNM Reacton
    ammo		= "Hedgehog Bomb",
	head		= "Walahra Turban",
    body		= "Yinyang Robe",
    hands		= "Summoner's Bracers +1",
    ...
    waist		= "Hierarch Belt",
	left_ear	= "Astral Earring",--padfoot
...
sets.daylight = { -- idle with daylight refresh stuff. lyco sash, feronia's bangles...
	hands		= "Feronia's Bangles",--crafted
...
sets.perp = { -- Avatar perpetuation cost-
	main 		= "Claustrum",-- 3
	body 		= "Penance Robe", --2
	head		= "Summoner's Horn +1",--3 on weather Spurrer Beret
	legs		= "Goliard Trews",--pet def
...
sets.perp.day = {body="Summoner's Doublet"}  		-- Perp cost- on day
...
sets.precast.siphon = { -- Summoning magic skill
	head		= "Evoker's Horn", -- Evokers Horn +5
	body		= "Yinyang Robe",
	hands		= "Summoner's Bracers +1",
	legs		= "Oracle's Braconi",
...
function blood_pact_information_for_party(name)
	...
	if name == "Dream Shroud" then
		...
		send_command('input /echo - ' .. name .. ': MAB +' .. tostring(hour) .. ', MDB +' .. tostring(14-hour) .. '')
	end
	...
	if name == "Lunar Cry" then
		...
		send_command('input /echo - ' .. name .. ': Accuracy -' .. tostring(moon) .. ', Evasion -' .. tostring(32-moon) .. '')
	end
	...
	if name == "Ecliptic Growl" then
		...
		send_command('input /echo - ' .. name .. ': STR/DEX/VIT +' .. tostring(moon) .. ', AGI/INT/MND/CHR +' .. tostring(8-moon) .. '')
	end
	...
	if name == "Ecliptic Howl" then
		...
		send_command('input /echo - ' .. name .. ': Accuracy +' .. tostring(moon) .. ', Evasion +' .. tostring(25-moon) .. '')
	end
...
function self_command(command)
	...
	elseif string.sub(command, 1, 2) == "bp" then -- check if the command begins with "bp" (start at 1, go to 2)
		execute_blood_pact(string.sub(command, 3, -1)) -- send the rest of the world to the function (start at 3, -1 means rest of the word)
